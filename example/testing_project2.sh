...
if [ -d "uploads/$test_project_name" ]; then
    ...
    echo `adb -s $test_device_serial_number shell am instrument -w -r -e emma true $apk_test_package/android.support.test.runner.AndroidJUnitRunner` >> testing_result/$test_project_name/$test_data/$test_device_serial_number/report
